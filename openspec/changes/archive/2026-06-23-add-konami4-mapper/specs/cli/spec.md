...
The CLI SHALL support compile mode selection flags: `-c`/`--compile` (Plain ROM), `-x`/`--megarom` (ASCII8 default), `-8`/`--ascii8` (ASCII8), `-k`/`--scc` (KonamiSCC), `-4`/`--konami` (Konami4), and `-a`/`--auto` (auto-fallback to ASCII8 on plain ROM overflow).
...
The `--help` flag SHALL list all compile mode flags including `-4`/`--konami`.

#### Scenario: Show help and exit cleanly
- **WHEN** the command `msxbas2rom -h` is executed
- **THEN** help text is printed
- **AND** the help text lists `-4`/`--konami` among compile options
