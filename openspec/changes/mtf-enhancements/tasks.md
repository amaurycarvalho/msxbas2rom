...
- [ ] 3.2 Add comment in `cmd_mtf.window_copy` VRAM address usage noting real page support is deferred to `set-page-screen4` (will replace `0x1800` with `(GRPNAM)`)
...
- [ ] 4.6a **Optimization**: skip LDIRMV when `screen_x == 0 && width == 32` (full-width rows — all bytes read would be overwritten by map data). Branch around the LDIRMV with a conditional check (~6 bytes of code). Full-screen copies (ops 0/1 delegation) benefit: only 1 VDP transfer instead of 2
...
- [ ] 5.1 After computing map_x/map_y (existing coordinate wrapping and modulo logic), write `MTF_COLX_PARM` and `MTF_ROWY_PARM` with computed values
- [ ] 5.2 Write `MTF_WIN_W_PARM = 32` and `MTF_WIN_H_PARM = 24` to workarea (full-screen dimensions)
- [ ] 5.3 Write `MTF_SCR_X_PARM = 0` and `MTF_SCR_Y_PARM = 0` to workarea (top-left screen origin)
- [ ] 5.4 Replace the 24-iteration `copy_to_buffer.loop` (ldir × 32 per row) with `jp cmd_mtf.window_copy`
- [ ] 5.5 Remove `cmd_mtf.map_xy.copy_to_buffer`, `cmd_mtf.map_xy.copy_to_buffer.loop`, `cmd_mtf.map_xy.copy_to_vram` labels and their code (~70 bytes eliminated)
- [ ] 5.6 Verify `cmd_mtf.map_xy.search_row_table` and `go_to_next_row` are preserved (window_copy reuses them)

## 6. Compiler Handler — Parameter Count Extension

- [ ] 6.1 Change max parameter check from `> 4` to `> 9` in `compiler_cmd_mtf_handler.cpp`
- [ ] 6.2 Validate minimum parameter count per operation (palette/tileset: 1–2, map op 0/1: 1–5, map op 2: 8–9)

## 7. Compiler Handler — RAM Block Emission

- [ ] 7.1 Replace register-based parameter passing with `ld (addr), hl` emits to DAC workarea offsets
- [ ] 7.2 Emit `xor a` / `ld (MTF_PAGE_PARM), a` as default for all omitted parameters (page=0, window=0)
- [ ] 7.3 Handle all 9 parameter positions: resource, op, col_x/map_x, row_y/map_y, width, height, screen_x, screen_y, page
- [ ] 7.4 Update kernel address constant `def_cmd_mtf` in `compiler_hooks.h` if the runtime grows past the 3-byte jump slot

## 8. Unit Tests

- [ ] 8.1 Add compiler test cases for all parameter counts 1–9 in `tests/unit/src/test_compiler.cpp`
- [ ] 8.2 Add compiler test cases for invalid parameter counts (0, 10+) expecting syntax error
- [ ] 8.3 Add compiler test cases verifying emitted opcodes write to correct DAC offsets
- [ ] 8.4 Run `make -C tests/unit run` to verify all tests pass

## 9. Integration Test

- [ ] 9.1 Verify `tests/integration/mtf/mtf5.bas` compiles successfully with the updated compiler
- [ ] 9.2 Confirm the existing `tests/integration/MTF/` tests (mtf1–mtf4) still compile and produce identical ROM output
- [ ] 9.3 Run `make -C tests/integration run` to verify all integration tests pass

## 10. Build Verification

- [ ] 10.1 Run `make release` (incremental) and confirm clean compilation
- [ ] 10.2 Verify `make debug` also compiles without forced clean
- [ ] 10.3 Confirm no regression in `make test-unit`
