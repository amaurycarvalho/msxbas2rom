...
- **Q2 resolution**: `getKernelCallAddr()` is updated to resolve targets through the `dw` dispatch table instead of checking for `0xC3` (`jp`) opcode bytes. All C-side wrapper calls go through a single helper (`addKernelCall()`) using the dispatch index, eliminating the need for a separate `addKernelDispatch()` emission method
- **Kernel size constraint**: The final kernel ROM SHALL NOT exceed `0x4000` bytes; a build-time check alerts if this limit is breached
- `def_wrapper_routines_map_start` is preserved at `0x4102` as the dispatcher base address
...
- **Affected C/C++ files**: `compiler_hooks.h` (DISP_ constants), `compiler_code_optimizer.h/.cpp` (getKernelCallAddr update, addKernelCall unification)
