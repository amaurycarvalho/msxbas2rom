...
Defines the requirements for routing C++ compiler calls to Z80 kernel routines through the word-pointer dispatch table. The wrapper table must cover every kernel routine reachable from compiled code, use symbolic labels exclusively, and be indexed via contiguous `DISP_*` constants in `compiler_hooks.h`. Named BIOS constants replace magic-number addresses for external calls.
