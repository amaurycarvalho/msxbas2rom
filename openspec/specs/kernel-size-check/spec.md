...
Establishes a hard kernel ROM size ceiling of 0x4000 bytes (16 KB) — the address space available between 0x4000 (ROM start) and 0x8000 (next page boundary). A build-time verification prevents the kernel from silently exceeding the Z80 page budget.
