...
Defines the word-pointer dispatch table mechanism in the Z80 kernel that replaces the linear `jp` jump table at `wrapper_routines_map_start` (0x4102). This enables index-based dispatch from C compiler code to kernel wrapper routines, saving ROM space (~120 bytes) for new kernel features.
