...
- [ ] Refactor src/application/lexer/lexer_line implementing a State pattern with minimum code change. The individual states files must be saved to src/application/lexer/states/ folder. Finally, adjust the related Makefile and unit testings according the changes;
...
- [ ] Search the codebase src/ folder for any code access to the file system. Next, refactor it to use a file system wrapper class into src/infrastructure/fs (pay attention to the file access pecularities used by each original code). Finally, adjust the related Makefile and unit testings according the changes.
