...
std::shared_ptr<ResourceReader> ResourceFactory::create(string filename) {
  string fileext = getFileExtension(filename);
  if (ResourceTxtReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(new ResourceTxtReader(filename));
  }
  if (ResourceCsvReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(new ResourceCsvReader(filename));
  }
  if (ResourceScrReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(new ResourceScrReader(filename));
  }
  if (ResourceSprReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(new ResourceSprReader(filename));
  }
  if (ResourceAkmReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(new ResourceAkmReader(filename));
  }
  if (ResourceAkxReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(new ResourceAkxReader(filename));
  }
  if (ResourceMtfReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(new ResourceMtfReader(filename));
  }
  if (ResourceMtfPaletteReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(
  ...
  if (ResourceMtfMapReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(new ResourceMtfMapReader(filename));
  }
  if (ResourceBlobReader::isIt(fileext)) {
    return std::shared_ptr<ResourceReader>(new ResourceBlobReader(filename));
  }
  return nullptr;
}
