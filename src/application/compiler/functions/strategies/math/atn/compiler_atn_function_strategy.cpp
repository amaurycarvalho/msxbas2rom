...
int AtnCompilerFunctionStrategy::execute(shared_ptr<CompilerContext> context,
                                         shared_ptr<ActionNode> action,
                                         int* result, unsigned int parmCount) {
  auto& optimizer = *context->codeOptimizer;
  if (!context || !action || !action->lexeme) return Lexeme::subtype_unknown;
  if (parmCount != 1) return Lexeme::subtype_unknown;
  if (action->lexeme->value != "ATN") return Lexeme::subtype_unknown;
  ...
  if (result[0] == Lexeme::subtype_single_decimal ||
    ...
    optimizer.addKernelCall(DISP_XBASIC_ATN);
