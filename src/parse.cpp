//-------------------------------------------------------------------
// Parser class specialized as a MSX BASIC syntax tree builder
...
Parser::Parser() {
    ...
    lex_index = new Lexeme(Lexeme::type_keyword, Lexeme::subtype_numeric, "INDEX");
                ...
                if((lexeme = lexerLine->getNextLexeme())) {
                    if(lexeme->type == Lexeme::type_literal && lexeme->subtype == Lexeme::subtype_string) {
                        ...
                        if(!loadInclude(lexeme)) {
                            error_message = "INCLUDE file not found or with content syntax error";
                            return false;
                        }
...
bool Parser::eval_statement(LexerLine *statement) {
    ...
    if(lexeme) {
        ...
        if     (lexeme->value == "REM" ||
                lexeme->value == "CLS" || lexeme->value == "END" ||
                lexeme->value == "BEEP" || lexeme->value == "RANDOMIZE") {
            result = true;
    ...
    while(actionStack.size() > actionCount)
        actionStack.pop();
...
Lexeme * Parser::coalesceSymbols(Lexeme *lexeme) {
    ...
    if(lexeme) {
        if(lexeme->type == Lexeme::type_identifier) {
            for(i = 0; i < t; i++) {
                result = symbolList[i];
                if(result->type == lexeme->type && result->subtype == lexeme->subtype &&
...
bool Parser::eval_assignment(LexerLine *assignment) {
    ...
    if(lexeme) {
        ...
        if(lexeme->type == Lexeme::type_identifier ||
                (lexeme->type == Lexeme::type_keyword && lexeme->subtype == Lexeme::subtype_function)) {
...
bool Parser::eval_expression_push(LexerLine *parm) {
    ...
    while(lexeme) {
        ...
        if(lexeme->type == Lexeme::type_operator || lexeme->type == Lexeme::type_separator) {
            ...
            if( outputCount == 0 || unary ) {
                ...
                if(outputCount == 0 && lexeme->value == ")") {
                    ...
                    if(lexeme->value == "+" || lexeme->value == "-") {  // unary plus or minus
                        lexeme->isUnary = true;
            ...
            if(lexeme->value == "(") {
                ...
                if(lastWasFunction || lastWasIdentifier) {
                    ...
                    if(expressionList.empty()) {
                        eval_expr_error = true;
                        error_message = "Invalid FUNCTION or ARRAY declaration in expression";
                        return false;
                    }
                    ...
                    while( (lexeme = parm->getNextLexeme()) ) {
                        ...
                        if( lexeme->value == "(" ) {
                            ...
                            if(sepcount)
                                ...
                                if( ! eval_expression_push(&functionLexemes) )
                                    return false;
                                functionLexemes.clearLexemes();
                                parmcount ++;
                                break;
                    ...
                    if(!parmcount) {
                        eval_expr_error = true;
                        error_message = "Invalid FUNCTION or ARRAY declaration in expression (missing parameters?)";
                        return false;
                    }
            ...
            while( ! operatorStack.empty() ) {
                ...
                if( ( thisPreced <= stackPreced || next_lexeme->type == Lexeme::type_keyword ) && next_lexeme->value != "(" ) {
                    pushStackFromLexeme(next_lexeme);
                    operatorStack.pop();
                } else
                    break;
...
int Parser::getOperatorPrecedence(Lexeme *lexeme) {
    ...
    if(lexeme->value == "IMP")
        ...
        result = 6;
    else if(lexeme->value == ">" || lexeme->value == "<" || lexeme->value == "=" || lexeme->value == ">=" || lexeme->value == "<=" || lexeme->value == "<>")
        result = 7;
    else if(lexeme->value == "+" || lexeme->value == "-")
        result = 8;
    else if(lexeme->value == "*" || lexeme->value == "/" || lexeme->value == "\\"
            || lexeme->value == "MOD" || lexeme->value == "SHR" || lexeme->value == "SHL")
        result = 9;
    else if(lexeme->value == "^")
        result = 10;
    else
    ...
    if(lexeme) {
        if(lexeme->type == Lexeme::type_operator) {
            if     (lexeme->value == "NOT" || lexeme->isUnary)
                result = 1;
            else if(lexeme->value == "AND" || lexeme->value == "OR" || lexeme->value == "XOR" || lexeme->value == "EQV" ||
                    lexeme->value == "IMP" ||
                    lexeme->value == ">" || lexeme->value == "<" || lexeme->value == "=" || lexeme->value == ">=" || lexeme->value == "<=" || lexeme->value == "<>" ||
                    lexeme->value == "+" || lexeme->value == "-" || lexeme->value == "*" || lexeme->value == "/" || lexeme->value == "\\" || lexeme->value == "^" ||
                    lexeme->value == "MOD" || lexeme->value == "SHR" || lexeme->value == "SHL")
                result = 2;
            ...
            if     (lexeme->value == "NOT")
                result = 1;
                    ...
                    lexeme->value == "IMP" || lexeme->value == "MOD" || lexeme->value == "SHR" || lexeme->value == "SHL")
                result = 2;
            else
                result = lexeme->parm_count;
        }
...
bool Parser::eval_cmd_data(LexerLine *statement, Lexeme::LexemeSubType subtype) {
    Lexeme *next_lexeme;
    string stext="";
    int i, itext;
    char *s;
...
bool Parser::eval_cmd_dim(LexerLine *statement) {
    ...
    for(i = 0; i < t; i++) {
        ...
        if(!lexeme->parm_count) {
            error_message = "Invalid array declaration: DIM size parameter is missing";
            return false;
        }
    }
        ...
        switch(state) {
            ...
            case 3: {
                    if(next_lexeme->type == Lexeme::type_identifier || next_lexeme->type == Lexeme::type_literal) {
                        ...
                        lex_using[0] = new Lexeme(Lexeme::type_keyword, Lexeme::subtype_function, "USING$");
                        lex_using[1] = new Lexeme(Lexeme::type_separator, Lexeme::subtype_string, "(");
                        lex_using[2] = next_lexeme;
                        lex_using[3] = new Lexeme(Lexeme::type_separator, Lexeme::subtype_string, ",");
                        lex_using[4] = new Lexeme(Lexeme::type_separator, Lexeme::subtype_string, ")");

                        for(i = 0; i <= 3; i++)
                            parm.addLexeme(lex_using[i]);
            ...
            case 4: {
                    if(next_lexeme->type == Lexeme::type_separator && (next_lexeme->value == "," || next_lexeme->value == ";")) {
                        state = 0;
                        continue;
                    } else {
                        return false;
...
bool Parser::eval_cmd_sprite(LexerLine *statement) {
    ...
    if( (next_lexeme = statement->getNextLexeme()) ) {
        ...
        if(next_lexeme->type == Lexeme::type_keyword) {
            ...
            if(next_lexeme->value == "ON" || next_lexeme->value == "OFF" || next_lexeme->value == "STOP") {
        ...
        switch(state) {
            ...
            case 1: {
                    ...
                    if( next_lexeme->isKeyword("FOR")) {
                        ...
                        s += 3;
                        stext = *s;
                        next_lexeme = new Lexeme(Lexeme::type_literal, Lexeme::subtype_numeric, stext);
                        pushActionFromLexeme(next_lexeme);
                        state = 5;
                        continue;
                    } else {
            ...
            case 2: {
                    ...
                    if( next_lexeme->value == "INPUT" || next_lexeme->value == "OUT" || next_lexeme->value == "APP") {
                        pushActionFromLexeme(next_lexeme);
                        popActionRoot();
                        continue;
                    } else if( next_lexeme->type == Lexeme::type_keyword &&
                               (next_lexeme->value == "PUT" || next_lexeme->value == "END") ) {
                        continue;
                    } else if(next_lexeme->value == "AS") {
                        state = 4;
                        continue;
...
bool Parser::eval_cmd_pset(LexerLine *statement) {
    ...
    if(parm.getLexemeCount()) {
        ...
        if(parmCount > 3) {
            ...
            if( next_lexeme->type == Lexeme::type_keyword || next_lexeme->type == Lexeme::type_operator) {
        ...
        if(isKeyword) {
            next_lexeme = new Lexeme(Lexeme::type_literal, Lexeme::subtype_numeric, parmValue);
            pushActionFromLexeme(next_lexeme);
        } else {
            parm.setLexemeBOF();
            if(!eval_expression(&parm)) {
            ...
            if(next_lexeme->type == Lexeme::type_identifier) {
                if(next_lexeme->value == "B") {
                    ...
                    isKeyword = true;
                      ...
                      (next_lexeme->type == Lexeme::type_operator && next_lexeme->value != "+" && next_lexeme->value != "-")) {
                int operatorCode = gfxOperatorCode(next_lexeme);
...
bool Parser::eval_cmd_cmd(LexerLine *statement) {
    ...
    if(lexeme->value == "PLYLOAD" || lexeme->value == "PLYPLAY" || lexeme->value == "PLYSONG")
        has_akm = true;

    if(lexeme->value == "RESTORE")
        has_resource_restore = true;
