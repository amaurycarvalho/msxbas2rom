    ...
    if(cmd_cmd) {
        if(lexeme->type == Lexeme::type_separator && lexeme->value == ":") {
            ...
            if(force_turbo || turbo_on) {
                error_message = "Additional instructions not allowed in front of CMD in Turbo mode";
                return false;
            }
        } else {
            if(force_turbo || turbo_on) {
                if(lexeme->type == Lexeme::type_identifier) {
                    ...
                    result = sprintf((char*)s,",%i,%i,%i",'#',i & 0xFF, i >> 8 & 0xFF);
                } else if(lexeme->type == Lexeme::type_keyword) {
...
int Tokenizer::writeTokenText(Lexeme *lexeme, unsigned char *s) {
    strcpy((char *) s, lexeme->value.c_str());
...
void Tokenizer::print() {
    ...
    for(unsigned int i = 0; i < lines.size(); i++) {
        line = lines[i];
        if(line)
            printf("%i (%i bytes) = %s\n", line->number, line->length, line->text.c_str());
    }
}
...
const char *Tokenizer::keywords_text[] = { "ABS", "AND", "ASC", "ATN", "ATTR$", "AUTO", "BASE",
                                           "BEEP", "BIN$", "BLOAD", "BSAVE", "CALL", "CDBL", "CHR$",
                                           "CINT", "CIRCLE", "CLEAR", "CLOAD", "CLOSE",
                                           "CLS", "CMD", "COLOR", "COS", "CONT", "COPY", "CSAVE",
                                           "CSNG", "CSRLIN", "CVD", "CVI", "CVS", "DATA", "DEF",
                                           "DEFDBL", "DEFINT", "DEFSNG", "DEFSTR", "DELETE",
                                           "DIM", "DRAW", "DSKF", "DSKI$", "DSKO$", "ELSE", "END",
                                           "EOF", "EQV", "ERASE", "ERL", "ERR", "ERROR", "EXP",
                                           "FIELD", "FILES", "FIX", "FN", "FOR", "FPOS", "FRE",
                                           "GET", "GOSUB", "GOTO", "GO TO", "HEX$", "IF", "IMP",
                                           "INKEY$", "INP", "INPUT", "INSTR", "INT", "IPL", "KEY",
                                           "KILL", "LEFT$", "LEN", "LET", "LFILES", "LINE", "LIST",
                                           "LLIST", "LOAD", "LOC", "LOCATE", "LOF", "LOG", "LPOS",
                                           "LPRINT", "LSET", "MAX", "MERGE", "MID$", "MKD$", "MKI$",
                                           "MKS$", "MOD", "MOTOR", "NAME", "NEW", "NEXT", "NOT",
                                           "OCT$", "OFF", "ON", "OPEN", "OR", "OUT", "PAD", "PAINT",
                                           "PDL", "PEEK", "PLAY", "POINT", "POKE", "POS", "PRESET",
                                           "PRINT", "PSET", "PUT", "READ", "REM", "RENUM", "RESTORE",
                                           "RESUME", "RETURN", "RIGHT$", "RND", "RSET", "RUN", "SAVE",
                                           "SCREEN", "SET", "SGN", "SIN", "SOUND", "SPACE$", "SPC",
                                           "SPRITE", "SQR", "STEP", "STICK", "STOP", "STR$", "STRIG",
                                           "STRING$", "SWAP", "TAB", "TAN", "THEN", "TIME", "TO",
                                           "TROFF", "TRON", "USING", "USR", "VAL", "VARPTR", "VDP",
                                           "VPEEK", "VPOKE", "WAIT", "WIDTH", "XOR", "INPUT$", "SPRITE$",
                                           "?", "_", "INTERVAL",
                                           "RANDOMIZE", "SHL", "SHR", "INKEY",
                                           "RUNASM", "RUNBAS", "WRTVRAM", "WRTFNT", "WRTCHR", "WRTCLR", "WRTSCR",
                                           "WRTSPRPAT", "WRTSPRCLR", "WRTSPRATR", "RAMTOVRAM", "VRAMTORAM",
                                           "DISSCR", "ENASCR", "KEYCLKOFF", "MUTE",
                                           "PT3LOAD", "PT3PLAY", "PT3MUTE", "SETFNT", "CLRSCR", "RAMTORAM",
                                           "PT3LOOP", "PT3REPLAY", "CLRKEY",
                                           0
                                         };

const unsigned char Tokenizer::keywords_token[] = { ,
                                                    'a', 'b', 'c', 'd', 'e', 'f',
