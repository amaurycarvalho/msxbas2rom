...
400 J=0:ONINTERVAL=6GOSUB401:SOUND 7,&B110111 :SOUND8,15:COLOR=(2,0,0,0):INTERVALON:COLOR:COLOR=(2,0,0,0):SETPAGE1,1:COLOR15,0,0:GOTO 402
...
602 XS=8:C=CW:Y=0:D=1:A$="     O MSX 2.0    ":GOSUB5720:D=1:A$="‚ um computador semi-PROFISSIONAL....":GOSUB 5720
604 SETPAGESP,1:COLOR15,0:CLS:CIRCLE(380,100),110,14,,,2/3:PAINT(0,0),4,14:SETPAGE0,0
606 RESTORE 610
608 C=CR:C2=CF
610 DATA-3,14,-20,40,295,-35,œetiqueta\MSX 2.0\  œ
612 DATA20,-5,20,-20,-85,80,œ192œ Kb RAM\œ64œ Kb ROM
614 DATA30,18,45,30,-100,-20,œ253œ caract‚res\no teclado
616 DATA10,18,-45,45,154,-55,œ2œ conectores\Interface universal
618 DATA-2,0,-20,-20,264,80,œ24œ linhas\œde 80œ colunas de texto
620 DATA10,-4,10,-30,30,96,graficos em\œ256œ cores
622 DATA6,-2,-8,-32,170,110,œ512x212œ\pixels graficos
624 DATA20,18,28,56,0,-50,10 fun‡µes\program veis
626 DATA15,-3,30,-45,-36,113,œ3œ geradores\de voz e de sons
630 FORK=1TO9
...
660 DATAE DEPOIS MSX," UM PADR°O MUNDIAL,",DE PROGRAMAS,PROFISSIONAIS DISPON‰VEIS
...
802 COLOR 0,15,15:FORI=0TO256STEP2:COPY(I,0)-STEP(1,211),1 TO (I,0),0:COPY(511-I,0)-STEP(1,211),1 TO (511-I,0),0:NEXTI:COLOR:INTERVALOFF
...
805 RESTORE805:DATA"S±o Paulo 9 de Maio 1986",,,"      Cara  Angelica,",,,,"Este ¢timo tratamento de textos trabalhado em 80 colunas,","dispµe de um padr±o internacional de caract‚res,","e ‚ muito f cil de ser utilizado.","   ",Abra‡os,"     Felipe
...
953 IF F THEN LINE (P+9,Q)-STEP(0,-3):LINE-STEP(B-11,0):LINE-STEP(0,3):PAINT(P+15,Q-1),CC,1
...
1015 C=3:X=10:Y=0:A$="Aplica‡µes Comerciais...":GOSUB5775
...
1616 SETPAGE0,0:CIRCLE (C1,C2),R,2,,,VH:PAINT (C1,C2),CT,2:CIRCLE(C1,C2),R,1,0,PI,VH:FORI=8TO15:CIRCLE(C1,C2),R,I-1,PI*I/8,PI*(I+1)/8,VH:NEXTI
...
1648 NEXT I
1650 RESTORE1654:C=1:FORK=1TO6:D=1:X=P+197:Y=123+9*K:READA$:GOSUB 5700:NEXTK
1651 LINE(C1-40,C2-12)-STEP(80,25),4,BF:PUTSPRITE0,(0,209),0,0:PUTSPRITE1,(0,209),0,0
1652 RETURN
1654 'data
1656 DATA"Extremo Oriente, 1984 - 1986"
1658 DATA"----------------------------"
1660 DATA"Indon‚sia  -Borneo      16 %"
1662 DATA"Tailƒndia  -Bangkok     69 %"
1664 DATA"Birmƒnia   -Mandalay     8 %"
1666 DATA"Laos       -Vietn±       7 %"
1672 DATA140, 60,20
1674 DATA150,100,60
1676 DATA160,140,0
1800 '
...
2100 GOTO 40
3974 RETURN
3999 SETPAGE1,1:COLOR:COLOR=(0,0,1,5):COPY(0,0)-(511,211),1 TO (0,0),0:SETPAGE0,0:RETURN
5664 COLORC,0:PRESET(X  ,Y),0,TPSET:PRINT#1,A$
5700 '
5702 IF D THENPRESET(X+1,Y),0,TPSET:PRINT#1,A$:D=0
5704 Y=Y+9:RETURN
...
5754 IF MID$(A$,I,1)="œ" THEN X=X-XS:T=C:C=C2:C2=T:COLOR C,0:GOTO5756
5755 PSET (X+XS*I-XS,Y),0,TPSET:PRINT#1,MID$(A$,I,1)
5756 IF D THEN PSET (X+XS*I-XS-1,Y),0,TPSET:PRINT#1,MID$(A$,I,1)
5757 NEXTI
5758 Y=Y+9:D=0:RETURN
5775 SETPAGE0,1:COLOR C,0:PSET(X,Y),0,PSET
5777 PRINT#1,A$
5779 IFDTHEN PSET(X+1,Y),0,PSET:PRINT#1,A$:D=0
...
8062 CIRCLE(X1+10*S,Y1-S*18),S*4,C,1.82*PI,PI,1.7
8063 LINE(X1+12*S,Y1-S*16)-(X1,Y1-S*6),C
8064 CIRCLE(X1+10*S,Y1-S*18),S*10,C,1.7*PI,PI,1
...
8515 CIRCLE(X-30*S/4,Y+6*S),30*S/4,C,3.1416,0,.5
8520 CIRCLE(X-30*S/4,Y+6*S),26*S/4,C,,,.5
