...
; ### ROM header ###
	...
	dw INIT2	; Main program execution address.
	dw 0		; STATEMENT
	dw 0		; DEVICE
	dw 0		; TEXT
	dw 0,0,0	; Reserved

INIT2:	; Program code entry point label

	ld	a,32
	ld	(LINL32),a	; 32 columns
	call	INIT32		; SCREEN 1

	call	PAGE_1_ENABLE		; Select the ROM on page 4000h-7FFFh

MSG2:
	ld	hl,Page8000hTXT	; Text pointer
	call	Print		; Call the routine Print below
 
	jp	MSG1	; Jump to below page.
...
PAGE_1_ENABLE:
	...
	or	c
	ld	h,040h
	jp ENASLT	; Select the ROM on page 4000h-7FFFh
...
; ### fake ROM header ###
	...
	dw 0		; ID for auto-executable ROM
	dw 0     	; Main program execution address.
	dw 0		; STATEMENT
	dw 0		; DEVICE
	dw 0		; TEXT
	dw 0,0,0	; Reserved
